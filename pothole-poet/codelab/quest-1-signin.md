...
Every interaction with Google Cloud — clicking in the Console, running `gcloud`, calling an API from code — boils down to *"call API X in project Y as identity Z."* This page sets up two of those three: **your identity** (Volvo Cars SSO) and **your project** (the one printed on your workbench card). The third (an API call) happens for the first time in Q2.
