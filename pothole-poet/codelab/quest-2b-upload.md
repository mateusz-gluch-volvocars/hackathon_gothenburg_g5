...
- <strong>DAG appears with red error pill.</strong> Click into it to see the import error. If it says &ldquo;No such file: <code>sql/01_federate.sql</code>&rdquo; &mdash; re-run the upload step with the <code>-r</code> flag to include the directory.
...
- <strong><code>gcloud storage</code> errors with <code>access denied</code>.</strong> The Workstation SA needs <code>roles/storage.objectAdmin</code> on the bucket &mdash; pre-bound by the platform; flag a Sherpa if missing.
